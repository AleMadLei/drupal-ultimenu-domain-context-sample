...
class UltimenuBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function blockSubmit($form, FormStateInterface $form_state) {
    ...
    $object = $form_state2->getFormObject();
    if ($object && $entity = $object->getEntity()) {
      if ($entity->getPluginId() == 'ultimenu_block:ultimenu-main') {
        $config = $this->ultimenuManager->getConfigFactory()->getEditable('ultimenu.settings');

        if ($theme = $entity->getTheme()) {
          $config->set('themes.' . $theme, $entity->id());
          $config->save(TRUE);
        }
      }
    }
